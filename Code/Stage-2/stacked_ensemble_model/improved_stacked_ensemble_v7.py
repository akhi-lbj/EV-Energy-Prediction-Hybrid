...
# ====================== 2. 60:10:30 STRATIFIED SPLIT (every month in train + test) ======================
np.random.seed(42)
test_indices = []
val_indices = []
...
for month in range(1, 13):
    ...
    # Test = 30%
    test_sample = month_df.sample(n=int(0.3 * n_month), random_state=42)
...
print(f"Train (60%): {len(X_train):,} sessions (all months present)")
print(f"Validation (10%): {len(X_val):,} sessions")
print(f"Test (30%): {len(X_test):,} sessions (all months present)")
...
def run_stacked(with_behavioral=True):
    ...
    test_mae, test_rmse, test_r2 = eval_set(y_test, final_test, "Test (30%)")
    
    # Log
    results_row = pd.DataFrame([{
        'timestamp': datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        'experiment': f'Stacked_60_10_30_{suffix}',
        'n_train': len(X_train),
        'n_val': len(X_val),
        'n_test': len(X_test),
        'test_mae': test_mae,
        'test_rmse': test_rmse,
        'test_r2': test_r2,
        'note': '60:10:30 stratified split'
    }])
...
print("\n=== ABLATION SUMMARY (60:10:30 Split) ===")
...
print("\n✅ 60:10:30 Stacked Ensemble completed.")
